import { runSteps, ScriptStep } from '@xylabs/ts-scripts-yarn3'
  ...
  const cjsSteps: ScriptStep[] =
    target === 'cjs'
      ? [
          ...
          ['yarn', ['workspaces', 'foreach', '--exclude', pkgName, ...incrementalOptions, 'run', 'package-compile-cjs', ...verboseOptions]],
  ...
  const esmSteps: ScriptStep[] =
    target === 'esm'
      ? [
          ...
          ['yarn', ['workspaces', 'foreach', '--exclude', pkgName, ...incrementalOptions, 'run', 'package-compile-esm', ...verboseOptions]],
