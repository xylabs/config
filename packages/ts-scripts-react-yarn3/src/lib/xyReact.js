#!/usr/bin/env node
import { cycle, dead, deps, fix, genDocs, gitlint, gitlintFix, license, lint, lintFast, lintProfile, parseOptions, reinstall, relint, runRimraf, sonar, tsconfigGen, tsconfigGenClean, up, updateYarnPlugins, updo, xyInstallCommands, yarn3Only, } from '@xylabs/ts-scripts-yarn3';
...
export const xyReact = () => {
    return xyReactBuildCommands(xyInstallCommands(parseOptions()))
        ...
        .command('clean [package]', 'Clean - Remove intermediate files', (yargs) => {
        return yargs.positional('package', {
            describe: 'Specific package to clean',
        });
    }, (argv) => {
        if (argv.verbose)
            console.info(`Cleaning: ${argv.package ?? 'all'}`);
        ...
        if (argv.verbose)
            console.info(`License: ${argv.package ?? 'all'}`);
        process.exitCode = license();
    })
        .command('cycle [package]', 'Cycle - Check for dependency cycles', (yargs) => {
        return yargs.positional('package', {
            describe: 'Specific package to check',
        });
    }, (argv) => {
        ...
        if (argv.verbose)
            console.info('Relinting');
        process.exitCode = relint();
    })
        .command('reinstall', 'Reinstall - Clean & Install', (yargs) => {
        return yargs;
    }, (argv) => {
        if (argv.verbose)
            console.info('Reinstalling');
        ...
        if (argv.verbose)
            console.info('Sonar Check');
        process.exitCode = sonar();
    })
        .command('test', 'Test - Run all unit tests', (yargs) => {
        return yargs;
    }, (argv) => {
        if (argv.verbose)
        ...
        .command('tsconfig-gen [package]', 'Tsconfig Gen - Generate tsconfog.json file for building', (yargs) => {
        return yargs.positional('package', {
            describe: 'Specific package for generation',
        });
    }, (argv) => {
        if (argv.verbose)
            console.info(`TsconfigGen: ${argv.package ?? 'all'}`);
        process.exitCode = tsconfigGen({ target: argv.target });
    })
        .command('tsconfig-clean', 'Tsconfig Clean - Remove generated tsconfig.json files', (yargs) => {
        return yargs;
    }, (argv) => {
        if (argv.verbose)
            console.info('Tsconfig Clean');
        ...
        .command('rimraf', 'Run rimraf', (yargs) => {
        return yargs;
    }, (argv) => {
        if (argv.verbose)
            console.info('Rimraf');
        ...
        .command('up', 'Up - Update dependencies', (yargs) => {
        return yargs;
    }, (argv) => {
        if (argv.verbose)
            console.info('Up');
        ...
        .command('updo', 'Updo - Update dependencies [Interactive]', (yargs) => {
        return yargs;
    }, (argv) => {
        if (argv.verbose)
            console.info('Updo');
